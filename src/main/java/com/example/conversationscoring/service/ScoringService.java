...
public class ScoringService {
  ...
  public void processScoring(ScoringRequest request) {
    ...
    try {
      ...
      ScoringStatus completedStatus =
          new ScoringStatus(id, "COMPLETED", score, "Scoring completed successfully");
      statusStore.put(id, completedStatus);

    } catch (Exception e) {
      ScoringStatus errorStatus =
          new ScoringStatus(id, "FAILED", null, "Error processing scoring: " + e.getMessage());
